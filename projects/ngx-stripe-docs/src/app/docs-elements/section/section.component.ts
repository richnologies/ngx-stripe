...
@Component({
  ...
  template: `
    <div #sectionContainer class="flex-1 justify-center relative z-0 flex overflow-y-auto" style="height: calc(100vh - 4rem)">
      <main class="flex-1 m-auto py-4 max-w-screen-md h-full w-11/12 my-0 mx-1 max-w-2xl">
        <div class="my-6 prose prose-indigo text-gray-500 mx-auto">
          <ng-content select="[ngStrSectionMain]"></ng-content>
          <div class="!mt-8"></div>
          <ngstr-section-navigator></ngstr-section-navigator>
          <footer>
            <div class="max-w-5xl mx-auto py-12 lg:py-16">
              <div class="flex space-x-6 justify-between">
                <div class="flex-row grow gap-1">
                  <a
                    href="https://twitter.com/richnologies"
                    class="mr-4 inline-block text-gray-400 hover:text-indigo-500"
                    target="_blank"
                  ...
                  <a
                    href="https://github.com/richnologies/ngx-stripe"
                    class="mr-4 inline-block text-gray-400 hover:text-indigo-500"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                ...
                <p class="text-base text-gray-400 float-right">
                  Made with love by
                  <a href="https://ricardosanchez.dev/" class="text-gray-400 hover:text-indigo-500 no-underline" rel="noopener noreferrer" target="_blank">Ricardo Sánchez</a>
                </p>
              </div>
            </div>
          </footer>
      ...
      <aside class="hidden xl:block xl:pl-8 w-1/3 max-w-sm overflow-y-auto sticky top-0" *ngIf="aside">
        <div class="space-y-5 mr-8 mt-12">
...
export class NgStrSectionComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    merge(fromEvent(this.section.nativeElement, 'scroll').pipe(debounceTime(250)), fromEvent(window, 'resize'))

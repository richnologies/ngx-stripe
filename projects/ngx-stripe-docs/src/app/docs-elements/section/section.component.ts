  ...
  template: `
    <div class="flex-1 justify-center relative z-0 flex h-screen overflow-hidden">
      <main
        ...
        class="flex-1 m-auto py-10 max-w-screen-md h-full w-11/12 my-0 mx-1 overflow-y-auto max-w-2xl"
      >
...
export class NgStrSectionComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    merge(fromEvent(this.main.nativeElement, 'scroll').pipe(debounceTime(250)), fromEvent(window, 'resize'))
